...
describe('Hack#1 Private Test-1', () => {
    ...
    context('Problem 5', () => {
        it('5-1 Render green correctly', () => {
            const test = ['w','a','n','b','c','enter']
            for(let i=0; i < test.length; i+=1){
                cy.get('body').type(`{${test[i]}}`)
            }
            cy.get('.Keyboard-container > .Keyboard-row > #char_n').should('have.class', 'green').and('have.text', 'n')
            cy.get('.Keyboard-container > .Keyboard-row > #char_w').should('have.class', 'green').and('have.text', 'w')
